...
//		list.add(3.14); // 제네릭 타입 고정으로 인해 정수 외에 추가 불가
		...
		// 배열과 마찬가지로 존재하지 않는 인덱스 지정할 경우 예외 발생
//		System.out.println("4번 인덱스에 저장된 요소 : " + list.get(4));
		...
		// index 번호는 고정 = 제일 끝에 있는 3이 인덱스 번호3이라 3으로 출력된것
...
//		System.out.println("정수 2 삭제 : " + list.remove(2)); // java가 인덱스 2로 인식함
		// -> 요소값이 아닌 2번 인덱스로 지정되어 정수2가 아닌 2번 인덱스 정수 3이 삭제됨
//		System.out.println("list 객체에 저장된 모든 요소 : " + list);
...
//		System.out.println("list 객체에 저장된 모든 요소 : " + list);
		...
		list.remove(list.indexOf(2)); // -> "2"가 위치한 인덱스 리턴을 remove에 넣어줌
		System.out.println("list 객체에 저장된 모든 요소 : " + list);
		
		// add(int index, Object o) : 특정 인덱스에 해당 요소(o)를 추가
		// -> 기존 데이터는 뒤로 밀려남 (덮어쓰는 것이 아님!)
		list.add(1, 5); // 1번 인덱스에 5 추가 (기존 1번 데이터부터 뒤로 밀림)
		System.out.println("list 객체에 저장된 모든 요소 : " + list );
		
		// set(int index, Object o) : 특정 인덱스에 해당 요소(o)를 덮어씀
		list.set(1, 10); // 1번 인덱스에 10추가(기존 1번 데이터 덮어씀)
		System.out.println("list 객체에 저장된 모든 요소 : " + list );
		
		// List subList(int beginIndex, int endIndex) 
		// -> beginIndex 부터 endIndex -1 까지 부분 리스트 출력
		List subList = list.subList(1, 3); // 1번 인덱스 부터 2번 인덱스(3-1) 까지 부분 리스트 추출 
