...
public class Test {
	...
	public static void main(String[] args) {
		...
		// 같은 패키지 안에 public으로 된 Person이 있어서 사용 가능
		// Person 클래스 인스턴스 2개 생성
		Person p1 = new Person("홍길동", 20);
		Person p2 = new Person("강감찬", 40);
		
		System.out.println(p1); // p1.toString() 의 형태로 
		System.out.println(p2);
		
		System.out.println("==================================");
		
		// Person 객체 여려개를 하나의 객체에 저장하여 관리할 경우
		// 1. Object[] 배열(또는 Person[] 배열)을 통해 관리 
		// -> 생성된 배열의 크기가 불변이므로 추가되는 객체 저장 불가
		Object[] objArr = {p1, p2}; // Person -> Object 로 업캐스팅
		// 오른쪽이 부모타입 왼쪽이 자식타입 (업캐스팅) 오른쪽에서 왼쪽으로 이동된다
		// 부모클래스 타입으로 간다? -> 업캐스팅이 일어난다
		// 자식을 부모에게 보낸다 = 업캐스팅이 일어난다 
		// 참조변수는 접근이 가능하지만 그 안에 있는 멤버변수에는 접근이 불가능
...
//			Person p = objArr[i];  Object 타입을 넣는것이라 다운캐스팅 필수!
		...
		List list = new ArrayList(); // <> 지우면 Object로 처리
		list.add(p1);
		list.add(p2);
		// 배열이 아닌 Collection API 는 객체 추가가 자유로움(자동 크기 확장됨)
		list.add(new Person("모아나", 30));
		list.add("엘사"); // Object 타입이므로 Person이 아닌 타입도 추가 가능
		// 
...
//			}
			...
			// 배열형태로 된 ArrayList에 제네릭 타입 <Person> 지정 시 
			// Person타입만 들어갈 수 있음
			List<Person> list2 = new ArrayList<Person>();
			
			list2.add(p1);
			list2.add(p2);
			list2.add(new Person("올라프", 7));
...
//			list2.add("감자");
			...
			for(int i = 0; i < list2.size(); i++) {
				// get() 리턴타입이 Person 타입이 되므로 형변환 불필요
				// -> 또한, 형변환 전 타입 판별도 생략
