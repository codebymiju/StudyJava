...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		// max()
		System.out.println(Integer.max(num1, num2)); // 기본 데이터 타입
		...
		ch = 't';
		if(Character.isUpperCase(ch)) { // is라서 boolean 타입으로 넘겨줌
			System.out.println(ch + " : 대문자!");
			
			// 대문자 > 소문자로 변환
//			ch += 32; // 형변환 없이 자연스럽게 가능
//			System.out.println("소문자로 변환 결과 : " + ch);
			
			// ~ 으로 형변환 : toXXX() , 맞는지 확인 isXXX()
			System.out.println("소문자로 변환 결과 : " + Character.toLowerCase(ch));
		} else if (Character.isLowerCase(ch)) {
			System.out.println( ch + " : 소문자!");
			System.out.println("소문자로 변환 결과 : " + Character.toUpperCase(ch));
		} else if (Character.isDigit(ch)) {
			System.out.println(ch + " : 숫자!");
		} else if(Character.isWhitespace(ch)) { // Character.isSpace(ch) 사용 X
