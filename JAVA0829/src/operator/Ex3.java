...
//		a = a + 1; // 컴파일에러 발생! 
		...
		// -> 따라서 x2 의 값이 5가 먼저 y2에 저장된 후, x2의 값이 5 -> 6으로 증가
		System.out.println(x2 + ", " + y2); // 6, 5
		
		System.out.println("=====================================================");
		
		// 대입연산자와 결합 대신 출력문에 사용
		x2 = 5;
		System.out.println(++x2); // x2 값을 먼저 1 증가시킨 후 출력 (6 출력) 증감연산자 먼저 사용됨
		System.out.println(x2++); // x2 값을 먼저 출력 후 1 증가시킴 (6 출력) 증감연산자 나중에 사용됨
		...
		// ch + 1 결과값을 출력 (char -> int로 변환하여 정수 출력) 하지만,
		// ch 값을 직접 변경하지는 않았으므로
		System.out.println(ch); // 변경 전 값인 'A' 출력됨
		
//		System.out.println(ch++);
		System.out.println(++ch);
		// ch + 1 결과값을 문자 그대로 출력하면서, ch 값을 직접 변경 하였으므로
		System.out.println(ch); // 변경 후 값인 'B' 출력
